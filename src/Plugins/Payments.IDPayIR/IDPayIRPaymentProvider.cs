...
{
    public class IDPayIRPaymentProvider : IPaymentProvider
    {
        ...
        public IDPayIRPaymentProvider(
            ...
            IDPayIRPaymentSettings idPayIRPaymentSettings,
            IIdPayIrHttpClient idPayHttpClient)
        {
            ...
            _idPayIRPaymentSettings = idPayIRPaymentSettings;
            _idPayHttpClient = idPayHttpClient;
        }

        #endregion

        public string ConfigurationUrl => IDPayIRPaymentDefaults.ConfigurationUrl;

        public string SystemName => IDPayIRPaymentDefaults.ProviderSystemName;

        public string FriendlyName => _translationService.GetResource(IDPayIRPaymentDefaults.FriendlyName);

        public int Priority => _idPayIRPaymentSettings.DisplayOrder;
        ...
        private IdPayDataNeedForCreatePayment createIdPayRequest(Order order)
        {
            //get store location
            var storeLocation = _workContext.CurrentHost.Url.TrimEnd('/');

            return new IdPayDataNeedForCreatePayment {
                ...
                callback= $"{storeLocation}/Plugins/PaymentIDPayIR/PaymentHandler"
            };
        }
        ...
        public async Task PostRedirectPayment(PaymentTransaction paymentTransaction)
        {
            ...
            IdPayDataNeedForCreatePayment request = createIdPayRequest(order);
            var isTest = this._idPayIRPaymentSettings.UseSandbox;

            var response = await this._idPayHttpClient
                                .CreateIdPayPayment(this._idPayIRPaymentSettings.ApiToken, request, isTest:isTest);

            if (response.HasError) throw new Exception(response.ErrorMessage);

            await _userFieldService.SaveField(order, "IdPayPayment_Id_Link",response.PaymentTransactionId + "_" + response.PaymentLink);
        ...
        public async Task<double> GetAdditionalHandlingFee(IList<ShoppingCartItem> cart)
        {
            ...
            //if (_idPayIRPaymentSettings.AdditionalFee <= 0)
            //    return _idPayIRPaymentSettings.AdditionalFee;

            //double result;
            //if (_idPayIRPaymentSettings.AdditionalFeePercentage)
            //{
            //    //percentage
            ...
            //    result = (double)((((float)subtotal.subTotalWithDiscount) * ((float)_idPayIRPaymentSettings.AdditionalFee)) / 100f);
            //}
            //else
            //{
            //    //fixed value
            //    result = _idPayIRPaymentSettings.AdditionalFee;
            //}
            //if (result > 0)
            //{
        ...
        public async Task<string> Description()
        {
            ...
            return await Task.FromResult(_translationService.GetResource("Plugins.Payments.IDPayIR.PaymentMethodDescription"));
        }

        public void GetPublicViewComponent(out string viewComponentName)
        {
            viewComponentName = "PaymentIDPayIR";
        }

        public string LogoURL => "/Plugins/Payments.IDPayIR/logo.jpg";
