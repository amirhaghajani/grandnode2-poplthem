...
namespace Grand.Web.Themes.Popl.Features.Handels.Products
{
    public class MyGetProductOverviewHandler : IRequestHandler<MyGetProductOverview, IEnumerable<MyProductOverviewModel>>
    {
        ...
        public MyGetProductOverviewHandler(
        ...
        public async Task<IEnumerable<MyProductOverviewModel>> Handle(MyGetProductOverview request, CancellationToken cancellationToken)
        {
            ...
            var tasks = new List<Task<MyProductOverviewModel>>();
            ...
            var result = await Task.WhenAll<MyProductOverviewModel>(tasks);
            return result;
        }


        private async Task<MyProductOverviewModel> GetProductOverviewModel(Product product, MyGetProductOverview request,
        ...
        private MyProductOverviewModel PrepareProductOverviewModel(Product product)
        {
            var sename = product.GetSeName(_workContext.WorkingLanguage.Id);
            var model = new MyProductOverviewModel 
            {
                Id = product.Id,
        ...
        private async Task<IList<MyProductOverviewModel.MyProductAttributeModel>> PrepareAttributesModel(Product product)
        {
            var result = new List<MyProductOverviewModel.MyProductAttributeModel>();
            if (product.ProductAttributeMappings.Any(x => x.ShowOnCatalogPage))
            {
                foreach (var attribute in product.ProductAttributeMappings.Where(x => x.ShowOnCatalogPage).OrderBy(x => x.DisplayOrder))
                {
                    ...
                    if (pa != null)
                    {
                        var productAttributeModel = new MyProductOverviewModel.MyProductAttributeModel();
                        ...
                        //aha.com ----------------
                        productAttributeModel.Id = attribute.Id;

                        foreach (var item in attribute.ProductAttributeValues)
                        {
                            var value = new MyProductOverviewModel.MyProductAttributeValueModel();
                            value.Name = item.Name;
                            value.ColorSquaresRgb = item.ColorSquaresRgb;
                            

                            //aha.com -------------------
                            value.IsPreSelected = item.IsPreSelected;
                            value.Id = item.Id;
