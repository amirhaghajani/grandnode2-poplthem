...
function getHeaderHeight() {
    ...
    /*var h_nvbr=document.querySelector(".header-section .header"),
        h_nvbr_m=document.querySelector(".header-section #header"),
        body=document.body,
        extra_bar=0,
            annouce_slider_bar=0;
    
    if(body.classList.contains("gfp_shopper_toolbar--show") && $("#gfp_shopper_toolbar").length >0){
      extra_bar=$("#gfp_shopper_toolbar")[0].getBoundingClientRect().height;
    }
    
    if($("#announcement-bar-with-slider").length >0){
      $("#announcement-bar-with-slider").css("top",extra_bar);
      annouce_slider_bar=$("#announcement-bar-with-slider")[0].getBoundingClientRect().height;
    }
    
    jQuery("#header.mobile_nav-fixed--true, .header-fixed--true").css("top",extra_bar+annouce_slider_bar);
    
    var el_pad_Add=document.querySelectorAll(".content.main, .index-sections, #pagecontent");
    
    if(window.innerWidth>992){
      $(".content.main, .index-sections, #pagecontent").each(function(i,e){
        if(i==0)
            $(this).css({"padding-top":h_nvbr.getBoundingClientRect().height, "padding-bottom":0, display:"block"});
        else{
      ...
      $(".content.main, .index-sections, #pagecontent").each(function(i,e){
        if(i==0)
          $(this).css({"padding-top":h_nvbr_m.getBoundingClientRect().height, "padding-bottom":0, display:"block"});
        else{
          $(this).css("padding-top",0);
          $(this).css("padding-bottom",0);
        }
...
var header = {
    init: function () {
        ...
        // some custom correction for the ?ref=jason bar
        //const urlParam = new URLSearchParams(window.location.search);
        //jQuery("#header.mobile_nav-fixed--true, .header-fixed--true").css({top:'inherit'});

        var url = new URL(window.location.href);
        var vUrl = url.searchParams.get("ref");

        $("")
        /*
            if(Cookies.get('ref').trim()!='' ){
                document.body.classList.add("gfp_shopper_toolbar--show");
                    getHeaderHeight();
            }
        ...
        */
        ...
        if (is_touch_device() && $(window).width() <= 798 || $(window).width() <= 798) {
            ...
            $('.main_nav, .top_bar, .cart_container').on("mouseleave", function () {
                if ($(window).width() > 798) {
                    $('.dropdown_container').hide();
                    // clean css off dropdown menus
                    $('.dropdown_container .dropdown.menu').removeAttr("style");
                }
                $('.active_link').removeClass('active_link');
                $('.is-absolute').parent().addClass('feature_image');
            });
            ...
            /*$('.dropdown_link.icon-search').on("click", function(evt){
               evt.preventDefault();
               if(!$(this).hasClass('active_link')) {
                 var $dropdown = $(this).parents('.main_nav').find('[data-dropdown="' + $(this).data('dropdown-rel') + '"]');
       
                 $dropdown.find(".dropdown.menu").css({top:"15px",left:0+"px"});
            ...
            $('.dropdown_link').on('mouseover touchstart', function (e) {
                ...
                var $dropdown = $(this).parents('.main_nav').find('[data-dropdown="' + $(this).data('dropdown-rel') + '"]');
                var clickCount = $(this).attr('data-click-count');
                //get top and left of nav item;
                var nav_top = $(this)[0].getBoundingClientRect().top;
                var nav_left = $(this)[0].getBoundingClientRect().left;

                $dropdown.find(".dropdown.menu").css({ top: "15px", left: nav_left + "px" });

                $('.active_link').removeClass('active_link');

                if (!$(this).hasClass('active_link')) {
                    $dropdown.show();
                    if (jQuery(this).scrollTop() > 0 && jQuery("#gfp_shopper_toolbar").length > 0) {
                        jQuery("#header.mobile_nav-fixed--true, .header-fixed--true").css({ top: 51 });
                    }
                    else {
                        jQuery("#header.mobile_nav-fixed--true, .header-fixed--true").css({ top: 0 });

                        if (jQuery("#gfp_shopper_toolbar").length > 0) {
                            jQuery("#header.mobile_nav-fixed--true, .header-fixed--true").css({ top: 51 });
                        }
                    }
        ...
        $(".btn-close-search").on("click", function (evt) { $("body").removeClass("is-active"); $(".dropdown_container").hide(); });
...
$(function () {
    $('.image-grid-slider').flickity({ groupCells: true, autoPlay: true, pauseAutoPlayOnHover: true, cellAlign: 'left' });
    $('.image-grid-slider-new').flickity({ groupCells: true, autoPlay: true, pauseAutoPlayOnHover: true, cellAlign: 'left' });
})


// banner logo animation - 08/25/2021

// gsap.registerPlugin(ScrollTrigger);


// var timeline = gsap.timeline({paused: true});
// timeline.set(".banner-logo-wrapper", {className:"+=banner-logo-wrapper active"})
// timeline.fromTo(".banner-logo", {y: 120, rotation: 0.001}, {y:-7, rotation: 0.001}, 0)
// 		.to(".banner-logo img", {scale: 0.5,  rotation: 0.001}, 0);

// ScrollTrigger.matchMedia({
//   "(max-width: 798px)": function() {
//      ScrollTrigger.create({
//         trigger: ".banner-lottie-animation",
//         animation: timeline,
//         start: "top 25",
//         end: "175",
//         scrub: true,
...
$(window).scroll(function () {
    ...
    if (scroll >= 1) {
        $('.banner-logo-wrapper').addClass('active')
    } else {
        $('.banner-logo-wrapper').removeClass('active')
    }
})

/*$('body').on('DOMNodeInserted', '.ab-slide.active', function(e) {
    $('body').addClass('announcement--active');
});*/

$("body").bind("DOMSubtreeModified", function () {
    var announcement = $('.ab-slide.active');
